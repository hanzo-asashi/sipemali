        ...
        <div class="col-xl-4">
            <x-card>
                <x-card-header class="card-header">
                    <h4 class="card-title">
                        @if($showEditModal)
                            <span>Ubah Tipe Satuan</span>
                        @else
                            <span>Tambah Tipe Satuan</span>
                        @endif
                    </h4>
                </x-card-header>
                <x-card-body class="card-body">
                    <form wire:submit.prevent="{{ $showEditModal ? 'updateTipeSatuan' : 'createTipeSatuan' }}" class="needs-validation" novalidate>
                        @csrf
                        <div class="mb-3">
                            <label for="satuan" class="visually-hidden">Nama Tipe Satuan</label>
                            <input wire:model.lazy="satuan" type="text" id="satuan"
                                   class="form-control @error('satuan') is-invalid @enderror"
                                   name="satuan" placeholder="Nama Tipe Satuan"
...
{{--                                    Tambah</a>--}}
        ...
        <div class="col-xl-8">
            <x-card>
                <x-card-header>
                    <h4 class="card-title">List Tipe Satuan </h4>
                </x-card-header>
                <x-card-body class="card-body">
                    <div class="row">
                    ...
                    <div class="table-responsive mb-4">
                        <x-table class="align-middle table-hover table-check nowrap">
                            <x-table.table-head>
                                ...
                                <th scope="col">Nama Tipe Satuan</th>
                                <th style="width: 100px; min-width: 80px;">Action</th>
                            </x-table.table-head>
                            <x-table.table-body>
                                @forelse($listTipeSatuan as $tipe)
