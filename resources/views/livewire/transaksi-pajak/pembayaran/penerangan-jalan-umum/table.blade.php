<div>
    @section('title') Pembayaran Penerangan Jalan Umum @endsection
    @push('css')

    @endpush
    <x-breadcrumb>
        <x-slot name="li_1">Dashboard</x-slot>
        <x-slot name="title">Pembayaran Penerangan Jalan Umum</x-slot>
    </x-breadcrumb>
    <div class="content-header row">
        <div class="content-body">
            <div class="row align-items-center">
                <div class="col-md-6">
                    <div class="mb-3">
                        <h5 class="card-title">Semua Penerangan Jalan Umum <span class="text-muted fw-normal ms-2">({{ $listObjekPajak->count() }})</span>
                        </h5>
                    </div>
                </div>
            </div>
                    ...
                    <div class="row">
                        <div class="col-md-10">
                            ...
                            <div class="row">
                                ...
                                <div class="col-md-2">
                                    <div wire:ignore>
                                        <label class="form-label" for="search">Filter Tahun</label>
                                        <select wire:model="selectedTahun" class="form-select form-select-sm">
                                ...
                                {{--                        <div class="col-md-2">--}}
                                {{--                            <div wire:ignore>--}}
                                {{--                                <label class="form-label" for="search">Filter Berdasarkan Masa</label>--}}
                                {{--                                <select wire:model="selectedTriwulan" class="form-select form-select-sm">--}}
                                {{--                                    <option value="" selected>Semua Masa Pajak</option>--}}
                                {{--                                    @foreach($listTriwulan  as $key => $triwulan)--}}
                                {{--                                        <option value="{{ $key }}">{{ $triwulan }}</option>--}}
                                {{--                                    @endforeach--}}
                                {{--                                </select>--}}
                                {{--                            </div>--}}
                                {{--                        </div>--}}
                                ...
                                {{--                                    <option selected value="">Semua Status Bayar</option>--}}
                    ...
                    <x-table.table-body>
                        @forelse($listPembayaran as $op)
                            ...
                            <tr class="@if($this->isChecked($op->id)) bg-soft-light @endif">
                                ...
                                <td>
                                    <a href="{{ route('pembayaran.peneranganjalanumum.detail', $op->id) }}"
                                ...
                                <td>
                                    <span class="text-success">
                                        ...
                                        {{ !is_null($qryBln) ? $qryBln->tahun : setting('tahun_sppt') }}
                                    <br>
                                    <span class="small text-muted">
                                        Pembayaran Selanjutnya : {{ \App\Utilities\Helper::getNamaBulanIndo((string) $nextBln) ?: '-' }}
                                    </span>
                                </td>
                                <td class="text-center">
                                    <span class="badge badge-soft-{{ \App\Utilities\Helper::getAlertColor(!is_null($qryBln) ? $qryBln->status_bayar : 0) }}">
                                    ...
                                    <div class="d-flex flex-wrap gap-2">
                                        <div class="btn-group">
                                            ...
                                            @can('detail pembayaran')
                                                <a id="{{ $op->id }}" href="{{ route('pembayaran.peneranganjalanumum.detail', $op->id) }}"
...
{{--    @include('widget.modal-bayar')--}}
    @include('widget.modal-bayar-pju')
<!-- End Modal Update Pembayaran -->
    @push('script')
