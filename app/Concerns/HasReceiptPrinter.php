...
trait HasReceiptPrinter
{
    ...
    public function init($connector_type, $connector_descriptor, $connector_port = 9100)
    {
        $connector = match (strtolower($connector_type)) {
            ...
            'windows' => new WindowsPrintConnector($connector_descriptor),
            'network' => new NetworkPrintConnector($connector_descriptor),
            default => new FilePrintConnector('php://stdout'),
        };
...
//                'KENDARAAN: '.$this->store->getKodeKendaraan()
//            );
            $footer = "Terima Kasih atas kedatangan anda!\n";
