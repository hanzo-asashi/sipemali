...
class Table extends Component
{
    ...
    {
        if ($value) {
            $this->checked = $this->objekPajak->with(['wajibpajak', 'jenisObjekPajak', 'pembayaran'])
...
//    $this->authorize('delete', $this->checked);
        if ($tipe === 'bulk') {
            $pembayaran = $this->objekPajak->with(['wajibpajak', 'jenisObjekPajak', 'pembayaran'])
                ->where('id_jenis_op', $this->objekPajakId)
                ->whereKey($this->checked)->delete();
            $this->checked = [];

            return $pembayaran;
        } else {
            $objek = $this->objekPajak->with(['pembayaran'])->findOrFail($id);
            if ($objek) {
    ...
    {
        if (!is_null($value)) {
            $this->state['sisa'] = $this->state['jumlah_bayar'] - $this->state['nilai_pajak'];
        }
    }
...
//
        ...
        if ($create) {
            ...
            $this->closeModal();
            $this->resetInput();
            $this->redirectRoute('pembayaran.tambangmineral.detail', $this->objekPajak->id);
        } else {
            $this->alert([
                'success' => false,
...
//            ->groupBy('id_wp')
        ...
        return view('livewire.transaksi-pajak.pembayaran.tambang-mineral.table', compact(
            'listPembayaran','listBulan','listStatus','listTahun','listMetodeBayar'
        ));
    }
}
